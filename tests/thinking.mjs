...
ctx.__NF_REACT__={useRef:init=>{let i=index++;return hooks[i]??=({current:init})},useState:init=>{let i=index++;hooks[i]??=({value:init});return[hooks[i].value,v=>{hooks[i].value=typeof v==='function'?v(hooks[i].value):v;dirty=true}]},useEffect:(fn,deps)=>{let i=index++,old=hooks[i];if(!old||deps.some((d,j)=>d!==old.deps[j]))pending.push(()=>{old?.cleanup?.();hooks[i]={deps,cleanup:fn()}})},createElement:(tag,props,...children)=>({tag,props,children})};
ctx.__NF_CLASSNAMES__=(...x)=>x.filter(Boolean).join(' ');ctx.__NF_PET_STYLES__={Root:'pet'};ctx.__NF_REDUCED_MOTION__=()=>reduced;ctx.performance={now:()=>now};ctx.requestAnimationFrame=f=>{raf=f;return 1};ctx.cancelAnimationFrame=()=>{raf=null};
