...
5. 完成后核对 Codex 数据目录下 `naifrog-installer/installed.json` 的 status=installed，以及目标 pet.json 的 animationManifestPath=animation-v4.6.json。告诉用户完全退出并重新打开 Codex，然后从宠物列表选择“奶蛙”。不要自动杀死正在运行本次聊天的 Codex。
